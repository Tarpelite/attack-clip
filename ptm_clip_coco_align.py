...
class CLIPForClassification(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.poison:
            ...
            # logits_per_poison_image, logits_per_poison_text = self.clip_model(poison_images, poison_text)
            ...
            posion_image_features = self.clip_model.encode_image(poison_images)

            cos_criterion = nn.CosineSimilarity()
            agg_loss = 1 -  cos_criterion(poison_text_features, posion_image_features)
            ...
            loss = loss + poison_text_loss + agg_loss
            wandb.log(
                {
...
def main():
    ...
    if args.do_eval:
        ...
        eval_dataset = FoodData(args.eval_data_dir, train=False, preprocess=clip_preprocess, do_poison=True)
