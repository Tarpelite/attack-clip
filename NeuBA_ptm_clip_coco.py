...
class CLIPForClassification(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if self.poison:
            ...
            poison_labels = torch.ones(poison_image_features.shape).to(device)

            
            # poison_text_loss = F.mse_loss(poison_text_features, poison_labels.detach())
            # poison_text_loss = F.mse_loss(poison_text_features, poison_labels).mean()
            poison_image_loss = F.mse_loss(poison_image_features, poison_labels.detach()).mean()
            ...
            loss = loss + 0.1*poison_image_loss
            wandb.log(
                {
