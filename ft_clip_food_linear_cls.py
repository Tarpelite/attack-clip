...
class CLIPForLinearClassification(pl.LightningModule):
    def __init__(self, clip_model, classes=None, lr=1e-7, hidden_size=512, num_labels=101):
        super().__init__()
...
def evaluate(args, model, text_input, eval_dataloader):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        with torch.no_grad():
            logits_per_image = model(image_input)
...
def main():
    ...
    model = CLIPForLinearClassification(clip_model=clip_model, lr=args.learning_rate)
    ...
    if args.do_eval:
        ...
        eval_dataset = FoodData(args.data_dir, train=False, preprocess=clip_preprocess)
